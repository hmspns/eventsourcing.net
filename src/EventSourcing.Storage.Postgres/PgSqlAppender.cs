...
LIMIT $2 OFFSET $3;";
        ...
        public PgSqlAppender(IPayloadSerializer serializer, string connectionString, string schemaName)
        {
        ...
        public async Task<IAppendEventsResult> Append(StreamId streamName, IAppendDataPackage data, AggregateVersion expectedStreamVersion, CancellationToken cancellationToken = default)
        {
            try
            {
                await using (NpgsqlConnection conn = new NpgsqlConnection(_connectionString))
                {
                    ...
                    using (NpgsqlTransaction transaction = await conn.BeginTransactionAsync(IsolationLevel.Serializable, cancellationToken))
                    {
                        NpgsqlCommand selectVersionCommand = new NpgsqlCommand(string.Format(GET_VERSION_COMMAND, _schemaName, EVENTS_TABLE_NAME), conn, transaction);
                        selectVersionCommand.Parameters.AddWithValue(null, NpgsqlDbType.Varchar, streamName.ToString());
        ...
        private string BuildCommandText(StreamReadOptions readOptions)
        {
            ...
            {
                case StreamReadVolume.Data:
                    sb.AppendLine("SELECT id, tenant_id, stream_name, stream_position, \"timestamp\", payload_type, payload");
                    break;
                case StreamReadVolume.Meta:
                    sb.AppendLine("SELECT id, tenant_id, stream_name, stream_position, \"timestamp\", command_id, sequence_id, principal_id, payload_type");
                    break;

                default:
                    sb.AppendLine("SELECT id, tenant_id, stream_name, stream_position, \"timestamp\", command_id, sequence_id, principal_id, payload_type, payload");
                    break;
            }
            ...
            string where = readOptions.PrefixPattern switch
            {
                ...
                var p when p.Length > 1 => "WHERE " + string.Join($" {condition} ", p.Select(x => $"stream_name {like} '{x}%'"))
            };
            sb.AppendLine(where);
            sb.AppendLine("ORDER BY stream_position " + (readOptions.ReadDirection == StreamReadDirection.Forward ? "ASC" : "DESC"));
