...
internal abstract class PgSqlAppenderBase : IAppendOnly
{
...
";
    ...
    protected const string APPEND_EVENT_COMMAND =
        @"INSERT INTO ""{0}"".""{1}""
    ...
    protected const string GET_STREAM_DATA =
        @"
...
LIMIT $2 OFFSET $3;";
    ...
    protected readonly IPayloadSerializer _serializer;
    protected readonly NpgsqlDataSource _dataSource;

    protected PgSqlAppenderBase(IPayloadSerializer serializer, NpgsqlDataSource dataSource, string schemaName)
    {
    ...
    protected abstract NpgsqlBatchCommand GetInsertEventCommand(IAppendDataPackage data, IAppendEventPackage appendPackage, long position, object payload);

    protected abstract NpgsqlBatchCommand GetInsertCommandCommand(IAppendDataPackage data, object payload);
    ...
    public virtual async Task<IAppendEventsResult> Append(
        StreamId streamName,
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            await using NpgsqlCommand selectVersionCommand = await GetVersionCommand(conn, transaction, streamName, cancellationToken);
            ...
            foreach (IAppendEventPackage appendPackage in data.EventPackages)
            {
                ...
                NpgsqlBatchCommand cmd = GetInsertEventCommand(data, appendPackage, position, appendPackage.Payload);
                batch.BatchCommands.Add(cmd);
            }
            
            NpgsqlBatchCommand createCommandCmd = GetInsertCommandCommand(data, data.CommandPackage.Payload);
    ...
    public virtual async Task<IEventsData> ReadSpecificStream(StreamId streamName, StreamPosition from, StreamPosition to)
    {
    ...
    /// <returns>Events data.</returns>
    public virtual async Task<IEventsData> ReadAllStreams(StreamReadOptions readOptions)
    {
    ...
    public virtual async Task<StreamId[]> FindStreamIds(string startsWithPrefix)
    {
        await using NpgsqlConnection conn = await _dataSource.OpenConnectionAsync();
    ...
    private async Task<NpgsqlCommand> GetVersionCommand(
